/*!
 * parallaxkit.js — Motor avanzado de Parallax/Scroll v4.0
 * Autor: Maycoll (Maven Marketing)
 * Vanilla JS. Sin dependencias. GPU-friendly. Fallbacks completos.
 *
 * Cobertura:
 ...
 *
 *  API Pública: ParallaxKit.init(opts), .refresh(), .setConfig(next), .on(evt, cb), .off(evt, cb), .state
...
(function (global) {
  ...
  // Exponer
  global.ParallaxKit = ParallaxKit;
  App.ParallaxKit = ParallaxKit;

  // Auto-init seguro
  if (doc.readyState !== "loading") ParallaxKit.init();
  else on(doc, "DOMContentLoaded", () => ParallaxKit.init());
